...
const main = async () => {
  ...
  const tokenLock = accounts.tokenLock(state.publicKey, program.programId)[0];
  const userLock = accounts.userLock(state.publicKey, program.programId)[0];
  const userAta = await web3.getAssociatedTokenAddress(token, user.publicKey);
  const escrowAta = await web3.getAssociatedTokenAddress(token, escrow);
